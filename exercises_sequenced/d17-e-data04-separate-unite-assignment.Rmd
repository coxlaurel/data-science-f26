...
## TODO: pivot and separate here!
  pivot_longer(
    names_to = "varang",
    values_to = "value",
    cols = c(-thick, -rep) ## combines all cols excluding the subtracted ones
  ) %>%
    separate(
      col = varang,
      into = c("var", "angle"),
      ...
      pivot_wider(
        names_from = var, ## get new column names from exisiting column unique values
        values_from = value 
      )
