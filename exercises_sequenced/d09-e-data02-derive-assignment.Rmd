...
- They have the same output but version 2 uses a pipe.
...
- Why is `cov_diff` so large?
  - cov_diff is much larger because the ratio between diff_sd and diff_mean is so large.
...
**Observations**:
- Because cov is so high, there's a lot of variation in pricing across all cuts, which indicates that cut isn't a large factor in pricing diamonds.
