...
Other packages loaded with tidyverse include dplyr, forcats, ggplot2, lubridate, purrr, and more.


### __q3__ What are the main differences between `install.packages()` and `library()`? How often do you need to call each one?

install.packages() downloads the packages used in the library locally to your machine. library() points to where those packages can be found.
