...
- Write your observations here!
  - Good distinctness is the most represented distinctness in our data sitting at 46 total. 
  - Good distinctness also has the highest mean velocity and is nearly 50 km/s more than poor distinctness. 
- Why might your table differ from Michelson's? 
  - I believe my table is different from Michelson's because of my MeanVelocity type. Mine has one decimal, whereas Michelson's is an integer rounded to the nearest 10.
...
- Make a quantitative comparison between Michelson's uncertainty and his error.
  - The true error is about 3 times larger than Michelson's uncertainty.
...
- Similarities 
  ...
  - After Jun 16th, both plot mean velocity lines match with increase and decrease directions at the same dates.
  - Both plot mean velocity lines begin with an increase and plateau then decrease.
- Differences
  - Only real data has a trough that goes beneath the lower error limits.
  - The real data shows that velocity measured before Jun 16 is much more unreliable than velocity measured afterwards since the mean velocity line exceeds the adjusted for error lines.
  - The simulated data shows that velocity measured Jun 23 is more unreliable than data measured before since the mean velocity line exceeds the adjusted for error lines.
  - The simulated data overall is more consistent than the real data as there are fewer peaks and dips over time.
  - The simulated data holds plateaus longer than the real data, making the mean velocity line appear smoother.
...
- The velocity with the most spread between the upper and lower quartiles is surprisingly those with the good distinctness (3). The distinctness with the most consistent velocities actually come from 2.
- There are a few velocity value outliers with distinctness of 1 and 2.
- Velocity values of distinctness 1 or 3 have more on the lower half of their respective means.
...
- Velocity is grouped more consistently between the 60-70 degree temperature range. After 70 degrees, there's clearly much more variation in velocity which signals that temperature was impacting Michelson's results.
...
- Over time, the average velocity decreased and then plateaued. Perhaps there was an initial learning curve, but then Michelson got better at running consistent experiments. These inconsistencies would definitely impact his estimated speed of light. However, there aren't any clear patterns that explain the difference between Michelson's estimate and `LIGHTSPEED_VACUUM`.
